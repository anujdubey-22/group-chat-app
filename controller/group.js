...
exports.postCreateGroup = async (req, res, next) => {
  try {
    ...
    const admin = await user.addGroup(group, { through: Admin });
    // const admin = await user.addAdmin(group);
     console.log(admin, "admin in group controller");
    res.status(201).json({ message: "gorup create done", data: data });
  } catch (error) {
